...
func TestSession_InitSession(t *testing.T) {
	...
	sessionManager := s.InitSession()
	var sessKind reflect.Kind
	var sessType reflect.Type
	rv := reflect.ValueOf(sessionManager)
	...
	if sessType != reflect.ValueOf(sm).Type() {
		t.Error("wrong type returned testing cookie session. Expected:", reflect.ValueOf(sm).Type(), "got:", sessType)
	}
}
